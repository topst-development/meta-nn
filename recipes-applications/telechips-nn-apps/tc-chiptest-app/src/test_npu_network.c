...
static struct network_list net_list[] = {
    {.name = "/usr/share/ssdlite300_quantized", .dma_ref = 409881, .comp_ref = 611324},
    {.name = "/usr/share/yolov3_relu6_voc_quantized", .dma_ref = 6213799, .comp_ref = 7483465},
    {.name = "/usr/share/yolov4_waymo100k_3c_640_TDN_BNH_MIN32CH_quantized", .dma_ref = 5401625, .comp_ref = 8008437},
    {.name = "/usr/share/yolov4_waymo100k_3c_800_TDN_BNH_MIN32CH_quantized", .dma_ref = 7598888, .comp_ref = 12890035},
};

static void npu_print_regs(npu_t* npu);
...
int test_npu_network(npu_t* npu)
{
    ...
    npu_test_printf("========================\n");
    npu_test_printf("========================\n");
    npu_test_printf("NPU  Network Test\n");
    npu_test_printf("========================\n");
    ...
    for (i = 0; i < sizeof(net_list) / sizeof(net_list[0]); i++) {
        char* test_name;
        int input_size;
        int output_size;
#ifdef PROFILE_RUN
        ...
        sprintf(input_file_name,  "%s/sample/input.ia.bin", ref_path);
...
#ifdef PROFILE_RUN
        // run network in profile mode
        if (network_profile(net, in, out, &npu_perf) < 0)
        {
            npu_test_printf("%s - inference failed\n", test_name);
            goto test_fail;
        }

        npu_test_printf("elapsed time : %.2f in ms\n", 
    	    (float)npu_perf.elapsed_in_us / 1000);
        npu_test_printf("dma count    : %d @pclk\n", npu_perf.dma);
        npu_test_printf("comp count   : %d @pclk\n", npu_perf.comp);
#else
        // run network
...
static void npu_print_regs(npu_t* npu)
{
    ...
    for(i = 0; i < 64; i++) { 
        ...
        npu_test_printf("RD 0x%08x:0x%08x = base:data\r\n", base, data);
    }
}
